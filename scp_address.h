...
/*** TEMPORAL SCOPE CORE STRUCTURE ***/
struct temporal_core {
    int loop_id;
    int lb;
    int lw[50];
    int up[50];    
};
typedef struct temporal_core   tc_s;
typedef struct temporal_core*  tc_p;
...
int     cmpTSold(ts_p ts1, ts_p ts2);
int     cmpTS(ts_p ts1, ts_p ts2, tc_p tc);
int     cmpTSset(worklist_p tsSet1, worklist_p tsSet2, int lpId, tc_p tc);
...
int         cmpSAddr(saddr_p smem1, saddr_p smem2, int lpId, tc_p tc);
