...
/*Compare 2 TS, return 1 if tsSet1 == tsSet2, 0 otherwise Join Function*/
...
/*compare 2 scope with core constrains, return 0:inner 1:overlap -1:non-overlap*/
int cmpTS(ts_p bs1, ts_p bs2, tc_p tc) {
    ...
    for(x=0;x<tc->lb;x++)
    {
	printf("[%d,%d]",tc->lw[x], tc->up[x]);
	//printf("%d,%d",tc->lb,tc->loop_id);
	
    }*/

    //cmpTSold(bs1, bs2);

    if(bs1->loop_id == tc->loop_id ){ cmpTSold(bs1, bs2);
        //int j=1;
    	//for(j=0;j<tc->lb;j++){

    		//if(bs1->lw > tc->lw[1] && bs1->up < tc->up[1]&& bs2->lw > tc->lw[1] && bs2->up < tc->up[1]) {cmpTSold(bs1, bs2);}
		//if(bs1->lw < tc->lw[j] && bs1->up < tc->up[j]&& bs2->lw > tc->lw[j] && bs2->up < tc->up[j]) {
			//bs1->lw = tc->lw[j]; cmpTSold(bs1, bs2);}
		//if(bs1->lw < tc->lw[j] && bs1->up > tc->up[j]&& bs2->lw > tc->lw[j] && bs2->up < tc->up[j]) {
			//bs1->lw = tc->lw[j]; bs1->up = tc->up[j]; cmpTSold(bs1, bs2);}
		//if(bs1->lw < tc->lw[j] && bs1->up > tc->up[j]&& bs2->lw < tc->lw[j] && bs2->up < tc->up[j]) {
			//bs1->lw = tc->lw[j]; bs1->up = tc->up[j]; bs2->lw = tc->lw[j]; cmpTSold(bs1, bs2);}
		//if(bs1->lw < tc->lw[j] && bs1->up > tc->up[j]&& bs2->lw < tc->lw[j] && bs2->up > tc->up[j]) {
			//bs1->lw = tc->lw[j]; bs1->up = tc->up[j]; bs2->lw = tc->lw[j]; bs2->up = tc->up[j]; cmpTSold(bs1, bs2);}
...
int cmpTSold(ts_p as1, ts_p as2) {
...
int cmpTSset(worklist_p tsList1, worklist_p tsList2, int lpId, tc_p tc) {
    int         dbg = 0;
...
/*  compare two scoped addresses, return 
    ...
    + -1 if no iteraction*/
int cmpSAddr(saddr_p m1, saddr_p m2, int lpId, tc_p tc) {
    int tsCmp;
    ...
    tsCmp = cmpTSset(m1->tsList, m2->tsList, lpId, tc);
